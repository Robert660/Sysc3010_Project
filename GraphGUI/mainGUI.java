...
public class mainGUI extends JFrame /*implements ChangeListener*/{
	...
	private void createFrame() throws SQLException{
		...
		TimeSeriesCollection dataset1 = setAccelerometerData();
		...
		panel1.setPreferredSize(new java.awt.Dimension(screenSize.width/2, 355));
		//panel1.setMouseWheelEnabled(true);
		//panel1.setMouseZoomable(true);
		subPanel1.add(panel1, BorderLayout.NORTH);
		
		//Force Line Graph
		XYSeriesCollection dataset2 = setForceData();
		JFreeChart forceGraph = ChartFactory.createXYLineChart("Force Sensor Reading", "Time (ms)", "Force (N)", dataset2, PlotOrientation.VERTICAL, false, false, false);
		ChartPanel panel2 = new ChartPanel(forceGraph);
		panel2.setPreferredSize(new java.awt.Dimension(screenSize.width/2, 355));
	...
	private ResultSet connection() throws SQLException {
		...
		results = statement.executeQuery("select * from sensordata;");
		return results;
	}
	
	private TimeSeriesCollection setAccelerometerData() throws SQLException{
		//Accelerometer Line Graph
		TimeSeries accelR = new TimeSeries("Accelerometer");
		ResultSet r = connection();
		Second current = new Second();
		while(r.next()){ 
			accelR.add(current, r.getDouble("acceleration"));
			current = (Second)current.next();
		}
		return new TimeSeriesCollection(accelR);
	}
	
	private XYSeriesCollection setForceData() throws SQLException{
		XYSeries forceData = new XYSeries("Force Sensor");
		ResultSet r = connection();
		double i = 0;
		while(r.next()){
			forceData.add(i, r.getDouble("force"));
			i++;
		}
		return new XYSeriesCollection(forceData);
	}
	/*
	private ResultSet getAccel(){
		return accelData;
	}
	
	private ResultSet getForce(){
		return fData;
	}*/
